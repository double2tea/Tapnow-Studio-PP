...
自原作者 [@zhengxinlan1995-code](https://github.com/zhengxinlan1995-code) 的 2.6.1 版本以来，使用 Antigravity 从原始版本 2.6.1 到 3.8.0-rc.3（截至 2026-01-16）共经历 **126 次** 快速迭代与修复（含 73 个公开版本号及 53 次内部逻辑微测），包含 **5 个** 里程碑版本与 **三次** 重构。目前已进化为功能完善的工作流设计生产平台。
...
### 📌 最新候选版 (RC)
*   **文件名**: `Tapnow Studio-V3.8.0-rc.3.html`
*   **版本号**: **v3.8.0-rc.3** (2026-01-16)
*   **说明**: 3.8 预发布版本，详细变更请见 Changelog。

### 🚀 最新稳定版 (Latest Sync)
*   **文件名**: `Tapnow Studio-V3.7.36c.html`
*   **版本号**: **v3.7.36c** (2026-01-14)
*   **主要更新**: 
    *   ✅ **智能分镜计时器功能**: 0.1s 级刷新更顺滑，任务完成/失败停表并保留耗时，重生成从 0 开始。
    *   ✅ **锁定镜头批量修复**（继承 V3.7.31）：锁定镜头不再进入批量队列。
    *   ✅ **预览回填修复**（继承 V3.7.30）：分镜生成后预览图稳定回填。
    *   ✅ **1006 错误修复**：积分不足与错误码判定更准确，减少误判与重复请求。
    *   ✅ **支持首尾帧**：视频模式下首尾帧输入与显示稳定可用。
    *   ✅ **本地缓存回退**：预览链路 localCache > thumbnail > original。
    *   ✅ **本地保存去重**：同图按子文件夹隔离查重。

### 📁 关键里程碑版本 (Milestones)
| 版本 | 迭代次数 | 标题 | 核心特性 |
|------|----------|------|----------|
| **V3.7.34** | **MILESTONE** | **Timer Stabilized** | 计时器 0.1s 刷新与完成停表，耗时稳定保留。 |
