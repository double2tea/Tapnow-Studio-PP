...
- **[调整] 生成节点模型双层选择**: 生成角色/场景图视频节点改为双层选择（待验证）。
- **[调整] 本地缓存路径与提示**: 移除 .tapnow_cache 兼容，统一 history，路径斜杠统一、提示常驻（待验证）。
- **[调整] 保存命名与多图**: 保存按“项目名-提示词-编号”，多图缓存强制唯一 ID（待验证）。
- **[调整] 保存去重**: 本地保存同图去重，按子文件夹隔离查重（待验证）。
- **[调整] 缓存去重与补全**: 多图缓存补全并复用同 URL 缓存避免重复下载（待验证）。
- **[调整] 性能模式按钮解耦**: 顶部性能模式与历史闪电互不联动（待验证）。
- **[调整] 双层下拉对齐**: 下拉上对齐 / 上拉下对齐，避免文字跳动（待验证）。
- **[调整] 批量素材交互**: 后端缓存标记、小图编号、双击预览、底部区域勾选（待验证）。
- **[调整] 本地缓存回退**: 预览链路 localCache > thumbnail > original，Lightbox/历史兜底（待验证）。
- **[调整] 本地缓存校验**: 本地文件缺失时清理“本地”标记与缓存映射（待验证）。
- **[调整] 角色图片提示词**: 图片模式移除口播描述，避免文字出现在画面（待验证）。
- **[调整] 角色/场景视频分辨率**: 增加 720/1080/不选 选项并记忆分辨率（待验证）。
- **[调整] 角色/场景视频时长校正**: Jimeng 时长强制 5/10s，避免 15s 报错（待验证）。
